...
import Styles from "./mobile-team.module.scss";
...
export const MobileTeam: React.FC<Props> = ({
  players,
  streamerAccountId,
  gameModId
}) => {
      ...
      <tbody>
        {players.map((player) => (
          <tr key={player.GodName}>
            <td title={player.GodName} className={Styles["god-img"]}>
              <img src={cdnGodIconHelper(player.GodName)} alt="" />
            </td>
            <td align="center" style={{ minWidth: "122px" }}>
              <strong
            ...
            <td
              align="center"
              title="Wins/Losses"
              className={Styles["content-winlose"]}
            >
              {player.Rank_Stat &&
